...
a hosted vendor, an LLM-observability console you self-host on another host. **None of that
belongs in this compose project**, so the `FELIX_OTEL_*` settings are passed through
`compose.yml` itself: put them in `.env` and `make up` exports, with no overlay.
...
renders a chat as a generation with model, token usage and cost rather than as a bare span.
That is "Protocols, not vendors" applied to telemetry: an endpoint and a header, and nothing
in the harness knows which product is on the other end.

A backend is something Felix *sends to*, not something it hosts. An earlier
`compose.memoturn.yml` ran one vendor's API, worker and console inside this project, on
Felix's own Postgres, Valkey and MinIO — 8 services, its own database bootstrap, blob
bucket and reverse proxy, and the two heaviest containers in the stack. It was removed in
favour of the six lines above. Run that product from its own compose project, or use its
cloud, and point `FELIX_OTEL_ENDPOINT` at it.

### Sending to two places at once

A process has one OTLP destination. To fan out, run `make up-observability` — the overlay
points Felix at the local collector — and add a second exporter to
`config/otel-collector.yaml`. Fanning out is the collector's job, not the application's.
Note that it validates every *defined* exporter, so only add one once its credentials are
set; an exporter reading an unset variable crash-loops the collector even when no pipeline
references it.
