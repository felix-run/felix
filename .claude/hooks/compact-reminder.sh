...
- A test that cannot fail is worse than no test. For an import-driven test: ./scripts/prove-fails.sh <target> => PROVEN=failed (evidence), VACUOUS=passed (pins nothing), BROKEN=errored (the test is wrong). It shadows PYTHONPATH and changes nothing on disk, so a test that READS the tree (every AST/rglob invariant) is proven by mutation instead: introduce the violation, watch it go red, revert.
