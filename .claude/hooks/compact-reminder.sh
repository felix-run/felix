...
- A test that cannot fail is worse than no test: ./scripts/prove-fails.sh <target> runs it against pre-change source. PROVEN=failed (evidence), VACUOUS=passed (pins nothing), BROKEN=errored (the test is wrong).
