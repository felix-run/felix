...
# `docs/` is a prefix, so it lives outside the `$`-anchored group: an anchored `docs/` matches
# only a path that is literally the string "docs/", which no file is, so the gate was
# unsatisfiable by the very directory it tells you to update.
printf '%s\n' "$changed" | grep -qE '^(README\.md|CLAUDE\.md|CHANGELOG\.md|\.env\.example|deploy/GOVERNANCE\.md|deploy/.*/README\.md)$|^docs/' && exit 0
